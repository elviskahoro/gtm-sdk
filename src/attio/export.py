...
def is_attio_health_failure(result: ExecutionResult) -> bool:
    """Return True if ``result`` failed because Attio looked unhealthy.

    Inspects the last (failed) outcome's envelope for error_type values that
    indicate transport failures or Attio-side outages, as opposed to caller
    data errors (validation, missing fields, etc.).
    """
    if result.success or not result.outcomes:
        return False
    last = result.outcomes[-1]
    if last.success:
        return False
    return any(
        err.error_type in _ATTIO_HEALTH_FAILURE_ERROR_TYPES
        for err in last.envelope.errors
    )


def execute_with_breaker(
    plan: Iterable[AttioOp],
    breaker,  # libs.reliability.circuit_breaker.CircuitBreaker — typed loosely to avoid an import cycle for adapter-only consumers
) -> ExecutionResult | None:
    """Execute ``plan`` under a circuit breaker.

    Returns ``None`` immediately if the breaker is open (caller should
    translate to a 503 response). Otherwise runs ``execute(plan)`` and
    feeds Attio-health-related outcomes back into the breaker.

    Data-failure outcomes (validation errors, etc.) are intentionally
    neither counted as successes nor failures — they're orthogonal to
    Attio's transport health.
    """
    if not breaker.allow_call():
        return None
    result = execute(plan)
    if is_attio_health_failure(result):
