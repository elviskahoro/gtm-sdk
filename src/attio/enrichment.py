...
def backfill_company_domains_via_exa(
    *,
    ...
    Raises:
        ValueError: If neither or both selectors are set, either is empty,
            ``limit`` is negative, or ``sleep_seconds`` is negative.
    """
    ...
    # so we must reject invalid inputs before building the ext_tam iterator.
    # Mirror the model's validators so direct-call and Modal-boundary callers
    # see the same contract (roborev finding).
    ...
    # Short-circuit ``limit == 0`` BEFORE touching the iterator. Otherwise
    # we'd consume the first id from ``iter_company_ids_by_filter`` and
    # trigger an Attio query the caller explicitly asked us not to make
    # (roborev finding). Negative limits are rejected above; ``limit == 0``
    # remains a valid "do nothing" signal for dynamic callers.
    if limit == 0:
        return report
...
def _process_one_company(
    *,
    ...
    try:
        ...
        report.unresolved += 1
        return
    except AttioConflictError:
        # The PATCH conflicted with another record's unique domain. From this
        # row's perspective it's a race-like noop — we can't fill the slot
        # because the cluster won't allow it. Record as skipped_race so the
        # operator sees the volume without it being treated as a real failure.
        report.outcomes.append(
            CompanyDomainOutcome(
                company_record_id=company_record_id,
                company_name=company_name,
...
class BackfillCompanyDomainsQuery(BaseModel):
    ...
    def validate_limit(cls, v: int | None) -> int | None:
        # Accept ``limit == 0`` so model and orchestrator agree on contract:
        # zero is a valid "do nothing" signal handled by a short-circuit in
        # ``backfill_company_domains_via_exa``. Only negatives are caller
        # errors (roborev finding).
