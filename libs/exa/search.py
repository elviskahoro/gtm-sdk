...
def search(input: SearchInput, api_key: str | None = None) -> SearchResponse:
    ...
    try:
        if needs_contents_endpoint:
            # search_and_contents is deprecated in favor of search(), but plain
            # search() does not populate response.output for output_schema
            # extractions — see the routing comment above. Keep using the
            # deprecated method until the SDK exposes output on search().
            sdk_response = client.search_and_contents(**request_dict)  # pyright: ignore[reportDeprecated]
        else:
