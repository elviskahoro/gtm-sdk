        ...
        for individual_file_data in source_file_data:
            try:
                yield DestinationFileData(
                    string=individual_file_data.base_model.etl_get_json(  # trunk-ignore(pyright/reportAttributeAccessIssue,pyrefly/missing-attribute)
                        storage=storage,
                    ),
                    path=f"{bucket_url}/{individual_file_data.base_model.etl_get_file_name()}",  # trunk-ignore(pyright/reportAttributeAccessIssue,pyrefly/missing-attribute)
                )
...
def test_destination_file_data_with_storage_parameter() -> None:
    ...
    class MockModel(BaseModel):
        ...
        def etl_get_json(self, storage: BaseModel | None = None) -> str:
            if storage:
                return f'{{"name": "{self.name}", "storage_config": "{storage.config}"}}'  # trunk-ignore(pyright/reportAttributeAccessIssue,pyrefly/missing-attribute)
