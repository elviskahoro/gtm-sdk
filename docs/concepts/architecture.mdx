...
layering is enforced by **tach** (statically, in CI) and in review — not just described
here. See [Module boundaries](/contributing/module-boundaries) for the tach setup.
...
<Note>
  ...
  `[tool.setuptools.packages.find]` in the matching `pyproject.toml` (root, or
  `cli/pyproject.toml` for CLI packages).
</Note>
...
- `data-gen/` currently ships one product (`marketplace`).
