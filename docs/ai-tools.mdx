...
1. **Structured success data is JSON on stdout; errors and logs go to stderr.** Parse
   stdout only for commands whose reference documents structured output.
...
The full contract, including the `--json` payload override for commands that list it, is
on the [CLI contract](/concepts/cli-contract) page.
