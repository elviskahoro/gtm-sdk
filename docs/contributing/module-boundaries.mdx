...
2. **In CI** — The `trunk-check.yml` workflow's `Trunk Check` step runs the normal hold-the-line checks, followed by an explicit full-graph `trunk check --filter=tach --all` step. This ensures module boundaries are checked even when a change does not touch a Python file. Violations block merges. The check runs with `exact = true` and `root_module = "forbid"`, so unused `depends_on` entries and undeclared packages both fail CI.
...
- **`exclude`** — Paths to skip (tests, docs, scripts, `deploy.py`, etc.)
- **`modules`** — Explicit layer definitions and utility declarations
- **`exact`** — Fail when `depends_on` lists declare imports the code does not make (keeps allowlists honest)
...
When you add a real import across modules, update `depends_on` (or run `uv run tach sync --add`) in the same change. See the `tach.toml` source for the full configuration.
