...
<Card title="Public interfaces are frozen">
Cross-module imports must match an `[[interfaces]]` expose entry in `tach.toml`. New deep imports fail until the interface (or call site) is updated.
</Card>
...
The import path is not in that module's `[[interfaces]].expose` list. Either:
...
When you add a real import across modules, update `depends_on` and `[[interfaces]]` (or run `uv run tach sync --add` for depends_on) in the same change.
