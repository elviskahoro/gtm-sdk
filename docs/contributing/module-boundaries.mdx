...
- **external path exclusions** — `scripts/**` is operational tooling and `data-gen/marketplace/**` is an optional product; both are excluded from runtime external-dependency ownership checks with rationale in CI and `tach.toml`.
...
uv run tach check-external --exclude 'scripts/**,data-gen/marketplace/**'
```
