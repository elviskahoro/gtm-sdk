...
- Scripts that need an isolated toolchain run their recipe in the committed Flox environment by default. Set `RUN_WITH_DAGGER=1` to opt into the shared `scripts/lib/container.py` wrapper, which executes against a prebuilt image derived from the same Flox manifest. `CONTAINER_PHASE` is wrapper-owned; `CONTAINER_IMAGE` overrides the image reference. Keep recipe steps single-sourced and update both executors through the shared helper.
