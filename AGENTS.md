...
host. Set `GTM_DEPLOY_VIA_FLOX=1` to skip Dagger and instead run `flox activate --dir . --mode run -- infisical run -- uv run modal deploy` on the host — used by `tests/scripts/test_deploy_webhook.py` so CI doesn't need a Dagger engine or real Modal credentials.

**Dagger does not work on Conductor cloud sandboxes, period** — its engine only ships as a privileged container on BuildKit/runc, and every `withExec` needs a nested runc container whose creation fails at the kernel level in those sandboxes (issue #284; do not reinvestigate). `GTM_DEPLOY_VIA_FLOX=1` is the documented path there: Flox (`.flox/env/manifest.toml`, which pins `git`/`uv`) pins the deploy toolchain via the Nix store instead of container namespaces, giving the same reproducibility guarantee Dagger provides on Mac/CI without needing nested-runc. Local Mac Dagger deploys are unaffected — this is a deliberate two-tier split (Dagger where it works, Flox only where it can't run), not a third parallel mechanism.
...
- **Wrap with `uv run modal deploy`, not bare `modal deploy`.** Bare `modal` runs outside the project venv and can't import `src.*` packages registered in `pyproject.toml` → `ModuleNotFoundError: No module named 'src.fathom'`. Applies inside the Dagger container and on the `GTM_DEPLOY_VIA_FLOX=1` host path.
- **The Flox deploy path has no Dagger-style secret cache to defeat.** `_content_addressed_secret_name` exists only because `dagger.set_secret(name, value)` caches downstream `with_exec` steps keyed on `name`, not the plaintext — a rotated token with the same name would silently replay a stale deploy. `flox activate -- infisical run -- ...` runs as a plain host subprocess with no such cache, so creds flow straight through `infisical run` with no hashing/content-addressing needed.
