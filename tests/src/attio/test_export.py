...
# ---------- execute_with_breaker / is_attio_health_failure ----------


def _transport_fail(error_type: str) -> ReliabilityEnvelope:
    return ReliabilityEnvelope(
        success=False,
        partial_success=False,
        action="failed",
...
def test_is_attio_health_failure_true_for_timeout() -> None:
    ...
    outcome = OpOutcome(
        ...
        success=False,
        record_id=None,
        envelope=_transport_fail("ReadTimeout"),
    )
    result = ExecutionResult(success=False, outcomes=[outcome], fail_index=0)
    assert is_attio_health_failure(result) is True


def test_is_attio_health_failure_false_for_data_error() -> None:
    from src.attio.export import ExecutionResult, OpOutcome, is_attio_health_failure
    ...
    outcome = OpOutcome(
        ...
        success=False,
        record_id=None,
        envelope=_transport_fail("ValidationError"),
    )
    result = ExecutionResult(success=False, outcomes=[outcome], fail_index=0)
...
def test_is_attio_health_failure_false_for_success() -> None:
    ...
    result = ExecutionResult(success=True, outcomes=[outcome])
    assert is_attio_health_failure(result) is False
...
def test_execute_with_breaker_short_circuits_when_open(monkeypatch) -> None:
    from src.attio.export import execute_with_breaker

    handler = MagicMock(return_value=_ok("meet-1"))
    monkeypatch.setattr("src.attio.export.OP_HANDLERS", {UpsertMeeting: handler})

    breaker = _StubBreaker(allow=False)
    result = execute_with_breaker([_meeting()], breaker)
...
def test_execute_with_breaker_records_success_on_happy_path(monkeypatch) -> None:
    from src.attio.export import execute_with_breaker

    handler = MagicMock(return_value=_ok("meet-1"))
    monkeypatch.setattr("src.attio.export.OP_HANDLERS", {UpsertMeeting: handler})

    breaker = _StubBreaker(allow=True)
    result = execute_with_breaker([_meeting()], breaker)
...
def test_execute_with_breaker_records_failure_on_transport_error(
    ...
    def raise_timeout(_op, _table):
        raise httpx.ReadTimeout("simulated")

    monkeypatch.setattr(
        "src.attio.export.OP_HANDLERS",
        {UpsertMeeting: raise_timeout},
    )
...
def test_execute_with_breaker_ignores_data_failures(monkeypatch) -> None:
    from src.attio.export import execute_with_breaker

    handler = MagicMock(return_value=_fail("bad payload"))
    monkeypatch.setattr("src.attio.export.OP_HANDLERS", {UpsertMeeting: handler})
...
def test_execute_with_breaker_integration_opens_real_breaker(monkeypatch) -> None:
    ...
    breaker = CircuitBreaker(
        name="test",
        config=CircuitBreakerConfig(consecutive_failure_threshold=3),
    )
    for _ in range(3):
        out = execute_with_breaker([_meeting()], breaker)
        assert out is not None and out.success is False
    assert breaker.state == State.OPEN
    assert execute_with_breaker([_meeting()], breaker) is None
